      ...
      const options: any = {
        ...
        handler: async function (response: any) {
          try {
            ...
            // C. Remove Only Purchased Items from Cart (Instead of clearing entire cart)
            // Create keys for purchased items
            const purchasedItemKeys = payload.items.map(item => `${item.sku}-${item.size}`);
            
            // Get current cart from localStorage
            const currentCart = JSON.parse(localStorage.getItem('cart') || '[]');
            
            // Filter out only the purchased items (keep everything else)
            ...
            // Note: If you have a CartContext with removeItems method, use:
            // removeItems(purchasedItemKeys);
            // This ensures both localStorage and React state stay in sync
